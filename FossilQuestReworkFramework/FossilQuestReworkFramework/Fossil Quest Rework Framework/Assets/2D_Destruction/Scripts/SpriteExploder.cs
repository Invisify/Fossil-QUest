...
public static class SpriteExploder {
    public static List<GameObject> GenerateTriangularPieces(GameObject source, int extraPoints = 0, int subshatterSteps = 0, Material mat = null, float gravityScale = 1f)
    {
        ...
        foreach (Triangle tri in voronoi.Triangles())
        {
            ...
            {
                pieces.Add(generateTriangularPiece(source, triangle, origVelocity, origScale, origRotation, mat, gravityScale));
            }
        }
        ...
        if (subshatterSteps > 0)
        {
            ...
            {
                morePieces.AddRange(SpriteExploder.GenerateTriangularPieces(piece, extraPoints, subshatterSteps, mat, gravityScale));
    ...
    private static GameObject generateTriangularPiece(GameObject source, List<Vector2> tri, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat, float gravityScale = 1f)
    {
    ...
    public static List<GameObject> GenerateVoronoiPieces(GameObject source, int extraPoints = 0, int subshatterSteps = 0, Material mat = null, float gravityScale = 1f)
    {
        List<GameObject> pieces = new List<GameObject>();
        ...
        foreach (List<Vector2> region in voronoi.Regions())
        {
            ...
            {
                pieces.Add(generateVoronoiPiece(source, clippedRegion, origVelocity, origScale, origRotation, mat, gravityScale));
            }
        }
        ...
        if (subshatterSteps > 0)
        {
            ...
            foreach (GameObject piece in pieces)
            {
                morePieces.AddRange(SpriteExploder.GenerateVoronoiPieces(piece, extraPoints, subshatterSteps, null, gravityScale));
                GameObject.DestroyImmediate(piece);
            }
        }
        else
    ...
    private static GameObject generateVoronoiPiece(GameObject source, List<Vector2> region, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat, float gravityScale = 1f)
    {
        //Create Game Object and set transform settings properly
        GameObject piece = new GameObject(source.name + " piece");
        piece.transform.position = source.transform.position;
        ...
        //SerializeMesh sm = piece.AddComponent<SerializeMesh>();
        //sm.Serialize();
