...
public class LineRendererSmootherEditor : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        for (int i = 0; i < Curves.Length; i++)
        {
            ...
            Handles.DotHandleCap(EditorGUIUtility.GetControlID(FocusType.Passive), position + startTangent, Quaternion.identity, 0.001f, EventType.Repaint);

            if (i != 0)
            {
                Handles.color = Color.blue;
                Handles.DotHandleCap(EditorGUIUtility.GetControlID(FocusType.Passive), nextPosition + endTangent, Quaternion.identity, 0.001f, EventType.Repaint);
            }
        ...
        // Apply look-ahead for first curve and retroactively apply the end tangent
        {
            ...
            Handles.color = Color.blue;
            Handles.DotHandleCap(EditorGUIUtility.GetControlID(FocusType.Passive), Curves[0].Points[2], Quaternion.identity, 0.001f, EventType.Repaint);
        }
    ...
    private void DrawSegments()
    {
        for (int i = 0; i < Curves.Length; i++)
        {
            ...
            for (int j = 0; j < segments.Length - 1; j++)
            {
                ...
                Handles.DotHandleCap(EditorGUIUtility.GetControlID(FocusType.Passive), segments[j], Quaternion.identity, 0.001f, EventType.Repaint);
            }

            Handles.color = Color.white;
            Handles.Label(segments[segments.Length - 1], $"C{i} S{segments.Length - 1}");
            Handles.DotHandleCap(EditorGUIUtility.GetControlID(FocusType.Passive), segments[segments.Length - 1], Quaternion.identity, 0.001f, EventType.Repaint);
